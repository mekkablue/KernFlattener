...
def filterKernPairs(kernPairs, font):
	pairSet = set()
	for line in worthKeepingText.splitlines():
		if not line:  # skip empties
			continue
		for i in range(0, len(line), 2):
			leftChar, rightChar = line[i], line[i + 1]
			leftGlyph, rightGlyph = font.glyphForCharacter_(ord(leftChar)), font.glyphForCharacter_(ord(rightChar))
			if leftGlyph and rightGlyph:
				pairSet.add((leftGlyph.name, rightGlyph.name,))
	filteredKernPairs = []
	for pair in kernPairs:
		if (pair[0], pair[1], ) in pairSet:
			filteredKernPairs.append(pair)
	print("reduced pairs from %d to %d" % (len(kernPairs), len(filteredKernPairs)))
	return filteredKernPairs
